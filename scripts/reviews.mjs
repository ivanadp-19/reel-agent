...
export async function recordFinal({draft, qcOk, projectId, outFile, dir, publicDir, jobId = String(Date.now())}) {
...
const runFf = (cmd, args) => new Promise((resolve) => {
...
export async function makeProxy(input, proxyOut, posterOut) {
  const p = await runFf('ffprobe', ['-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', input]);
  const durationSec = parseFloat(p.stdout) || 0;
  const enc = await runFf('ffmpeg', proxyArgs(input, proxyOut));
  ...
  await runFf('ffmpeg', ['-y', '-v', 'error', '-ss', at.toFixed(2), '-i', proxyOut, '-frames:v', '1', '-q:v', '6', posterOut]);
  return {durationSec};
}
