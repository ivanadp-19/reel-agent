...
export function acquireLock(dir, id, {pid = process.pid, owner = `pid ${pid}`, now = Date.now(), ttlMs = LOCK_TTL_MS} = {}) {
  fs.mkdirSync(dir, {recursive: true});
  const f = lockFile(dir, id);
  const mine = {pid, host: os.hostname(), owner, since: now, beat: now};
  try {
  ...
  const cur = read(f);
  const same = cur && cur.pid === pid && cur.host === mine.host;
...
export function releaseLock(dir, id, pid = process.pid) {
  const f = lockFile(dir, id);
  const cur = read(f);
  if (cur && cur.pid === pid && cur.host === os.hostname()) fs.rmSync(f, {force: true});
}
