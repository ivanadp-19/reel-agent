...
// Output: public/transcript.json = [{clipId, source, words:[{i, word, startMs, endMs}]}]
...
const out = clips.map((clip) => {
  let words = [];
  try { words = transcribeClip(clip, lang); } catch (e) { console.error(`SKIP ${clip.id}: ${String(e).slice(0, 120)}`); }
