...
export const Editor: React.FC<{onBackToStart: () => void}> = ({onBackToStart}) => {
  ...
  const inputProps = useMemo(
    () => ({clips, music, captions, brolls, graphics, mattes, accentColor, captionStyle}),
    [clips, music, captions, brolls, graphics, mattes, accentColor, captionStyle],
  );
  ...
  }, [meta, projectId, projectName, clips, music, captions, brolls, graphics, mattes, brollAssets, accentColor, lang, captionStyle]);
  ...
  const exportVideo = async (draft = false) => {
    ...
    try {
      const r = await fetch('/api/render', {method: 'POST', body: JSON.stringify({clips, music, captions, brolls, graphics, mattes, accentColor, captionStyle, draft})}).then((x) => x.json());
      pollJob(
        '/api/render', r.jobId,
