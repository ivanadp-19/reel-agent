...
4. **Hook (first 3 s)**: one `add_graphic` — `hook-stack` (2–4 stacked lines, one accent) or `big-word` with `behind: true` (then `prepare_mattes`). Keep it inside the safe zone and off the presenter's face: `add_graphic` and `validate` warn with a `y_pct` that clears it. Long text shrinks to fit the frame on its own.
5. **Labels and data**: `label-2tone`, `stat`, `chapter` anchored with `at_wid` on the word that names the room/feature/number. A graphic stays on screen for its whole duration even across cuts. One text graphic on screen at a time.
