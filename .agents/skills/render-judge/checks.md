...
| `pagination` | rule, when the profile wants one page per sentence: a page that closes a sentence before its last word. The shared pager (`src/paging.ts`) now ends a page at every sentence, abbreviations aside; before, a bonded capitalized pair such as "Carmen. Está" kept two sentences on one page in `vibem` | major | ⟲ `set_caption_style` with the same pack (re-pages and keeps word ids, tiers, emoji, timing and hand pages). A hand-typed page: `edit_caption` to one sentence. Never `delete_captions` / `add_caption` |
