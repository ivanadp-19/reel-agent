...
| `split-name` | Compound names split in the captions | The last word of a page and the first of the next are consecutive transcript words, with no full stop between. **rule** when they form a glossary term or a highlight span. **candidate** for a capitalized pair (Cap+Cap, Cap+number): capitals are a bad signal for compounds, so the judge looks at the frame at the page change. A pair that the wrap estimate breaks across lines inside a page is also a candidate | major / candidate | `edit_caption` on both pages to move the word (the same word count keeps the real timing; otherwise re-judge `sync`). `delete_captions` a page left empty |
| `overflow` | Caption text overflowing | heuristic. For each page, the CaptionTrack sizing with the shared width table (`src/textFit.ts`): the widest unbreakable unit (a bonded pair or a word × its tier scale) is wider than the frame even at the 40 px floor = blocker; more than 3 lines = major; shrunk below 75 % = minor. Always confirm on `frame_at video` at the page's time | blocker / major / minor | `edit_caption scale` (the script gives the scale that fits) or a shorter `text` |
...
| `spelling` | rule: a caption lost an accent the transcript has ("Esta" for "Está"), or the same word is written two ways across captions and graphics (the accented form wins) | major | `edit_caption text` / `edit_graphic props` (the script writes the corrected text) |
...
| `pagination` | rule, when the profile wants one page per sentence: a page that closes a sentence before its last word | major | `delete_captions` + two `add_caption` (the script computes the times) |
| `accent-size` | rule, when the profile wants accent = plain size: the preset's tier scale ≠ the plain words' scale | major | **escalate**: preset data is code (`src/captionPresets.ts`), not an agent edit |
...
| Check | Detection | Severity | Fix |
|---|---|---|---|
| `insert-missing` | rule. Every insert the script asks for (the plan's `INSERTS:` block, plus the known reel in the profile) needs a B-roll cue whose asset tags / desc / query / file name contain one of its keywords, or a graphic (super) whose text does, near its anchor word (−2 s … +4 s) when it has one. If the profile has `requireInserts` and the plan lists none, that is a major | blocker | `suggest_broll` → `add_broll at_wid`; `search_stock` only when the library has nothing; `add_graphic` location-tag / label with the text the script gives |
