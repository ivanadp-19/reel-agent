...
- Render jobs (`scripts/render-jobs.mjs` the queue, `scripts/render-runner.mjs` one render stage by stage): a render is a job persisted in `public/render-jobs/<id>.json`; `POST /api/render` answers with its id at once. Serial by default — one render at a time on the machine, counted over every backend sharing `public/` (`REEL_RENDER_WORKERS` = N for more; `renderPlan` in `scripts/render-queue.mjs` sizes the Chrome tabs, `REEL_RENDER_CONCURRENCY` overrides). Real progress: stage (preparing → bundling → rendering → encoding → finalizing → review), overall %, frame x/y, ETA. Heartbeat: a job whose render process dies is failed, one with no progress for `REEL_RENDER_STALL_SEC` (900) is killed and failed; a job whose backend died is re-queued once on the next start (its orphan render killed by pid after `/proc/<pid>/cmdline` names the job — never by pattern), then failed. Cancel: queued → at once, running → its process group is killed, no partial file. Finished jobs' state is pruned after `REEL_RENDER_JOBS_DAYS` (14) — the job files only, never the mp4s. Same surface everywhere: MCP `start_render` / `render_status` / `list_render_jobs` / `cancel_render` (and the blocking `render`), the CLI `npm run render -- start|status|list|cancel|prune` (`scripts/render-cli.mjs`; status/list read the disk with the backend down), the editor's Renders panel (an export in flight is picked up again when the project reopens). Routes: `/api/render`, `/api/render/<id>`, `/api/render-jobs[/<id>[/cancel]]`, `/api/render-jobs/prune`
- Master cache hook (the layered pipeline caches masters by hash — another branch): when `scripts/master-cache.mjs` exists the runner calls `lookupMaster({props, raw, draft}) → {file, key} | null` before rendering (a hit copies the master and skips bundling + rendering; a final still goes through loudness, QC and the review version) and `storeMaster({props, raw, draft, file})` after a fresh render (copy the file: finalize rewrites it). A hook that throws counts as a miss. Contract in the header of `scripts/render-runner.mjs`
