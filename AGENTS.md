...
- Access in public mode (`REEL_PUBLIC=1`, Railway; `server/http.mjs` `gate`): `x-reel-token` (`REEL_BACKEND_TOKEN`), basic auth (`REEL_AUTH_BCRYPT`), or the form login (`server/session.mjs`) — `GET /login` is the form, `POST /login` (urlencoded or JSON) checks the same bcrypt users and sets `reel_session`, an HMAC-SHA256-signed cookie (user + expiry, 7 days, httpOnly, SameSite=Lax, Secure over https); `/logout` clears it. Signing key: `REEL_SESSION_SECRET`, else the first token of `REEL_BACKEND_TOKEN` (rotating either signs everyone out). Failed logins: 5 per IP per minute, then 429 + Retry-After (in memory). A browser page load without credentials is redirected to `/login`; API calls keep the 401 basic-auth challenge. `/r/<token>` and `/api/ping` stay public
