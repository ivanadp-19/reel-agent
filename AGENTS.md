...
- `server/index.mjs` — local backend on 127.0.0.1:3333 (`REEL_PORT` for a second one): projects, uploads, jobs, render. Clip ingest (`server/ingest.mjs`): a browser upload (`POST /api/add-clip` with a body) answers 202 `{jobId}` as soon as the file is on disk and probes / remuxes or transcodes / thumbnails in the background — polled at `GET /api/add-clip/<jobId>` (`running` with a real ffmpeg % and a label → `done` with the clip, or `error`), so a slow transcode never outlives a proxy's timeout, and a browser that leaves after the upload still gets its clip (the editor resumes pending jobs from localStorage); the MCP's `?path=` and the reel CLI's ways (`?path=` with a user token through `openForUser`, `?upload=<id>` of a chunked part) stay synchronous (200 with the clip). One log line per ingest transition. Every export goes through one render queue — never chain renders by hand with `nohup`; submit them all and let the queue run them. `scripts/render-bench.mjs` measures it (waits for the machine to be free); numbers in `research/render-benchmark.md`
- Access in public mode (`REEL_PUBLIC=1`, Railway; `server/http.mjs` `gate`): `x-reel-token` (`REEL_BACKEND_TOKEN`), basic auth (`REEL_AUTH_BCRYPT`), or the form login (`server/session.mjs`) — `GET /login` is the form, `POST /login` (urlencoded or JSON) checks the same bcrypt users and sets `reel_session`, an HMAC-SHA256-signed cookie (user + expiry, 7 days, httpOnly, SameSite=Lax, always Secure in public mode); `POST /logout` clears it (GET is 405). Both POSTs need an `Origin` (or `Referer`) of this site — Host, the trusted proxy's `X-Forwarded-Host` or `REEL_PUBLIC_URL` — else 403 (login CSRF). Signing key: `REEL_SESSION_SECRET`, else the first token of `REEL_BACKEND_TOKEN` (rotating either signs everyone out); the MAC also covers the tail of the user's bcrypt hash, so changing a user's password in `REEL_AUTH_BCRYPT` (or removing the user) ends that user's sessions. `next` is a same-origin path only (no control characters). Login attempts (form and basic auth alike): 5 per IP per minute, counted before bcrypt (parallel requests cannot race past it), then 429 + Retry-After (in memory, idle IPs swept); an unknown user runs bcrypt too, against a dummy hash at the real hashes' cost. The client IP comes from `X-Forwarded-For` only behind `REEL_TRUST_PROXY` trusted hops (the Dockerfile sets 1 for Railway), else the socket. A verified basic-auth header is remembered 5 min while the user's hash is unchanged. `x-reel-token` is compared in constant time (`tokenOk`). A browser page load without credentials is redirected to `/login`; API calls keep the 401 basic-auth challenge. The media the editor reads by URL (`/exports/*` downloads, `/clips/*`, B-roll, music, assets… — `server/http.mjs` `isMediaPath` / `servePublic`) is open to any caller the gate lets through, the session cookie and basic auth included: GET and HEAD only (405 for writes), `Cache-Control: private`, a missing file a 404 (never the editor's page); local mode keeps loopback for everyone. `/r/<token>` and `/api/ping` stay public
...
- `cli/reel.mjs` — `reel`, the CLI an agent uses over SSH on the backend's box (flow and contract: `cli/AGENTS.md`): only the HTTP API, never the server's files; `--json` everywhere, errors `{error, code, hint}` with an exit code per family, `reel help --json`, retry-safe commands. Backend side: per-user tokens (`server/tokens.mjs`: sha256 only in `.reel-tokens.json` 0600, `/api/tokens` for admins = the backend token or an admin user token, `/api/whoami`; the gate names the caller and refuses a revoked `reel_` token even on loopback; `REEL_REQUIRE_TOKEN=1` closes anonymous loopback), path ingest for a user token only of that user's own files or world-readable ones (`openForUser`, read through the checked descriptor), resumable upload parts (`server/uploads.mjs`, `/api/uploads/<id>`, `/api/add-clip?upload=`), `GET /api/validate/<id>` (`mcp/checks.mjs`, the MCP's validate), `GET` / `PUT /api/projects/<id>/captions` (the pages with a revision — a hash of them, `server/captions-revision.mjs`; a PUT with a stale `expectedRevision` is 409 `revision_mismatch` and writes nothing), `POST /api/render {project_id}` built from the saved project, answering with its plan (`planRender`: complete or layers, and why; `?plan=1` queues nothing), one render per token user and disk / memory floors (`admitRender`: `REEL_RENDER_MIN_FREE_DISK_MB` 3072, `REEL_RENDER_MIN_FREE_MEM_MB` 1024, Linux), the finished file at `/api/render-jobs/<id>/file` (byte ranges), the captions job's pages in its status
...
- `src/guion.ts` — the client's script (guion) against the ASR: `set_guion` / the Captions tab store it on the project (`guion`; the captions job falls back to `REEL_GUION` / `public/guion.txt`). The captions job aligns the ASR words to it (sequence alignment over folded text — no accents, ñ → n, no punctuation; digits and Spanish number words by value) and takes the guion's wording with the ASR's timing: spelling, one ASR word that swallowed two ("acomodan" = "acomoda" + "a", split in proportion inside its span; pieces share the word id), two that are one ("sky pool" → "skypool", never across a cut). A near-miss is adopted only next to an exact match; the rest is left as heard and reported. The word keeps what the ASR heard in `asr`. Content conflicts (70 vs 60 invitados, another name) keep the AUDIO — César: the audio wins on content — and are never resolved: `validate` (and the render judge, as an advisory) reports `guion-conflict`, plus `guion-missing` / `guion-altered` / `guion-extra` as warnings and `guion-timing` (a reconciled word with a negative or overlapping time) as an error
...
- `reel <command>` (`cli/reel.mjs`; on the server `sudo ln -sf "$PWD/cli/reel.mjs" /usr/local/bin/reel`) — projects, clips, captions, validate, render, review links over the API with a per-user token; `reel help --json`, `cli/AGENTS.md`
...
- **Remote agents (MCP over HTTP).** A collaborator's Claude Code connects to the hosted backend with no shell on it:
  ...
  One token per client: append it to `REEL_BACKEND_TOKEN` (comma-separated; the first stays the primary the backend and its own MCP use) and restart; revoke by removing it — its open sessions stop at once, since every request is gated. Paths the tools take (`add_clips files`, `add_broll_assets`, `set_brand font_files`) are paths on the server, not on the client's machine: upload media through the editor first. Over `/mcp` a tool accepts only files under `public/` (symlinks resolved — never `.env` or anything else of the server; stdio keeps any path), and the backend's path ingest (`/api/add-clip?path=`, `/api/add-broll-asset?path=`) takes only the primary token. Tokens are compared in constant time (`server/http.mjs` `tokenOk`, the one helper for the gate, `/mcp` and path ingest). `/mcp` is gated before basic auth and the login cookie: neither opens it. The tools run inside the backend there, so none of them spawns synchronously: ffmpeg runs as an async child and the QC gate as `scripts/qc.mjs --json`.
